...
- [Setting Up a Project]({{< relref "skills/setting-up-a-project.md" >}}) — Author distinct `CLAUDE.md` and `AGENTS.md` files, capture runnable commands, and install populated project rules before coding.
- [Repairing Project Setup]({{< relref "skills/repairing-project-setup.md" >}}) — Audit an existing agent setup, repair only the gaps while preserving recorded decisions, and re-audit the full setup before reporting.
